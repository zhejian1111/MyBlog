...
import PostItem from './blender-item'
...
const getMoreLink = (len: number): React.ReactNode => {
  ...
  return (
    <NextLink href="/blender" passHref>
...
const getLatest = (data: typeof metadata, isLatest?: boolean) => {
  const postNode = data.find(item => item.name === 'blender')
...
const Blender: React.FC<PostsProps> = ({ isLatest = false }) => {
